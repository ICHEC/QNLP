...
int main(int argc, char **argv){
...
#else
    int len_input = 6;
    int len_auxiliary = len_input-1;

    int num_exps, m;
    std::size_t n;
    n = len_auxiliary + len_input + 1;                     // Number of qubit registers
    num_exps = 10;
    m = pow(2,n);       // Number of possible states
#endif
    ...
    vector<std::size_t> auxiliary(len_auxiliary);
    vector<std::size_t> target(1);
    for(std::size_t j = 0; j < len_input; j++){
        input[j] = j;
    }
    for(std::size_t j = 0; j < len_auxiliary; j++){
        auxiliary[j] = j + len_input;
    }
    target[0] = n-1;
    ...
    for(int exp = 0; exp < num_exps; exp++){
...
#ifdef CHEAT
        //circ.ApplyNCPauliX( input,  auxiliary,  target);
        circ.ApplyNCUnitary( input,  auxiliary,  target, U_not);
#else
        //circ.ApplyNCPauliX( input,  auxiliary,  target, len_input);
        circ.ApplyNCUnitary( input,  auxiliary,  target, U_not, len_input);
#endif
        ...
        cout << "|";
        for(int j = 0; j < len_auxiliary; j++){
            cout << output[auxiliary[j]];
        }
        cout << ">";
        cout << "|";
