...
namespace QNLP{
    ...
    template <class Mat2x2Type, class SimulatorType>
        class NCU{
            private:
            ...
            public:
                        ...
                        const unsigned int depth, const bool isPauliX)
                {
                    ...
                    assert (qReg.getNumQubits() >= qControlEnd);
                    assert (qReg.getNumQubits() >= qTarget);
                    ...
                    if (cOps >= 2){
                        ...
                        if ( ! isPauliX ){
                            ...
                            qReg.applyGateCU( sqrtMatricesU[local_depth], qControlEnd, qTarget );
                            ...
                            //Apply single qubit gate ops, and decompose higher order controls further
                            qReg.applyGateCU(sqrtMatricesX[local_depth], qControlEnd, qTarget);
                            applyNQubitControl(qReg, qControlStart, qControlEnd-1, qControlEnd, sqrtMatricesX[0], 0, true );
                    ...
                    //If the number of control qubits is less than 2, assume we have decomposed sufficiently
                        ...
                        qReg.applyGateCU(U, qControlEnd, qTarget); //The first decomposed matrix value is used here
                    }
                }
